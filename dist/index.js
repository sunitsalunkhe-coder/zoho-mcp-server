...
async function createMcpServer(userId) {
    const server = new Server({ name: "zoho-mail-mcp", version: "1.0.0" }, { capabilities: { tools: {} } });
...
async function main() {
    ...
    if (transport === "stdio") {
        ...
        app.post("/mcp", requireBearerAuth({ verifier: oauthProvider }), async (req, res) => {
            const userId = req.auth?.extra?.userId ?? "default";
        ...
        app.get("/mcp", requireBearerAuth({ verifier: oauthProvider }), async (req, res) => {
        ...
        app.delete("/mcp", requireBearerAuth({ verifier: oauthProvider }), async (req, res) => {
            const sessionId = req.headers["mcp-session-id"];
            if (sessionId && sessions.has(sessionId)) {
